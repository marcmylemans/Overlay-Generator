...
(function (root, factory) {
  ...
  var KEYS = ['slate', 'chapter', 'terminal', 'tip', 'credit', 'bug', 'compare', 'checklist', 'members'];
  ...
  var META = {
    ...
    members:   { name: 'Member thanks',      pos: 'full frame' }
  };
  ...
  var FILES = {
    ...
    compare: '07_comparison-slide', checklist: '08_numbered-list', members: '09_member-thanks'
  };
  ...
  var DEFAULTS = {
    ...
    members:  { eyebrow: 'Channel Members', title: 'Thank You',
                tier1Title: 'Network Architect', tier1Names: 'Ada L.',
                tier2Title: 'Systems Specialist', tier2Names: 'Grace H., Linus T.',
                tier3Title: 'Tech Enthusiast', tier3Names: 'Dennis R., Ken T., Margaret H., Tim B-L.' }
  };
  ...
  // names entered comma- OR newline-separated → array of non-empty, trimmed names
  function listNames(s) { return String(s == null ? '' : s).split(/[\n,]/).map(function (t) { return t.trim(); }).filter(Boolean); }
  ...
  var DRAW = {
    ...
    members: function (ctx, d) {
      brandBackdrop(ctx);
      var cx = W / 2, sep = '   •   ';
      var eyeSize = 40, titleSize = 130, tierSize = 52, nameSize = 50, ruleH = 8;
      var ruleGap = 26, ruleToNames = 30, tierGap = 70, headGap = 72, nameLH = nameSize * 1.32;
      ctx.textAlign = 'center'; ctx.textBaseline = 'top';

      // visible tiers, ordered top → bottom (highest first)
      var tiers = [
        { title: d.tier1Title, names: d.tier1Names },
        { title: d.tier2Title, names: d.tier2Names },
        { title: d.tier3Title, names: d.tier3Names }
      ].map(function (t) { return { title: t.title, names: listNames(t.names) }; })
        .filter(function (t) { return t.names.length; });

      // measure
      font(ctx, titleSize, 700, false);
      var titleLines = wrap(ctx, d.title, W - 1000);
      var blocks = tiers.map(function (t) {
        font(ctx, nameSize, 500, false);
        var nameLines = wrap(ctx, t.names.join(sep), 2700);
        var hasTitle = !!(t.title && t.title.trim());
        var h = (hasTitle ? tierSize + ruleGap + ruleH + ruleToNames : 0) + nameLines.length * nameLH;
        return { title: t.title, hasTitle: hasTitle, nameLines: nameLines, h: h };
      });
      var headH = (d.eyebrow ? eyeSize + 28 : 0) + titleLines.length * titleSize * 1.04 + headGap;
      var tiersH = blocks.reduce(function (a, b) { return a + b.h; }, 0) + Math.max(0, blocks.length - 1) * tierGap;
      var y = Math.max(120, (H - (headH + tiersH)) / 2);

      // header
      if (d.eyebrow) {
        font(ctx, eyeSize, 600, false); ctx.fillStyle = C.sky; ls(ctx, '10px');
        ctx.fillText(d.eyebrow.toUpperCase(), cx, y); ls(ctx, '0px'); y += eyeSize + 28;
      }
      font(ctx, titleSize, 700, false); ctx.fillStyle = '#fff'; ls(ctx, '-2px');
      titleLines.forEach(function (ln, i) { ctx.fillText(ln, cx, y + i * titleSize * 1.04); });
      ls(ctx, '0px'); y += titleLines.length * titleSize * 1.04 + headGap;

      // tiers
      blocks.forEach(function (b, i) {
        if (b.hasTitle) {
          font(ctx, tierSize, 700, false); ctx.fillStyle = C.sky; ls(ctx, '8px');
          ctx.fillText(b.title.toUpperCase(), cx, y); ls(ctx, '0px');
          y += tierSize + ruleGap;
          var rw = 180, rg = ctx.createLinearGradient(cx - rw / 2, y, cx + rw / 2, y);
          rg.addColorStop(0, '#2563eb'); rg.addColorStop(1, '#38bdf8');
          ctx.fillStyle = rg; rr(ctx, cx - rw / 2, y, rw, ruleH, ruleH / 2); ctx.fill();
          y += ruleH + ruleToNames;
        }
        font(ctx, nameSize, 500, false); ctx.fillStyle = '#dbe4f0';
        b.nameLines.forEach(function (ln) { ctx.fillText(ln, cx, y); y += nameLH; });
        if (i < blocks.length - 1) y += tierGap;
      });
